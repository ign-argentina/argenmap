(function (factory) {
  ...
  function GeoJSONLoader(data, control) {
    ...
    var layer = L.geoJson(data, {
      ...
      pointToLayer: function pointToLayer(feature, latlng) {
        ...
        var sym = (prop.sym ? prop.sym : name)
          .replace(" ", "-")
          .replace('"', "")
          .replace("'", "")
          .toLowerCase(); // generate appropriate icon symbol or retrieve it from cache

        wptIcons[sym] = wptIcons[sym]
          ? wptIcons[sym]
          : L.divIcon({
      ...
      onEachFeature: function onEachFeature(feature, layer) {
        ...
        layer._latlngs.forEach(function (point, i, data) {
          ...
          if (feature.properties) {
            var prop = feature.properties;
            if ("coordTimes" in prop)
              point.meta.time = new Date(Date.parse(prop.coordTimes[i]));
            else if ("times" in prop)
              point.meta.time = new Date(Date.parse(prop.times[i]));
            else if ("time" in prop)
              point.meta.time = new Date(
                Date.parse(
                  _typeof(prop.time) === "object" ? prop.time[i] : prop.time,
                ),
              );
            if ("heartRates" in prop)
              point.meta.hr = parseInt(prop.heartRates[i]);
            else if ("heartRate" in prop)
              point.meta.hr = parseInt(
                _typeof(prop.heartRate) === "object"
                  ? prop.heartRate[i]
                  : prop.heartRate,
  ...
  function waitHolder(elem) {
    return new Promise(function (resolve, reject) {
      ...
      if (elem)
        elem.addEventListener("mouseenter", scrollFn, {
          once: true,
        });
      scrollFn();
    });
  ...
  function addClass(targetNode, className) {
    if (targetNode)
      className.split(" ").every(function (s) {
  ...
  function toggleClass(targetNode, className, conditional) {
    return (conditional ? addClass : removeClass).call(
      null,
      targetNode,
      className,
    );
  ...
  function style(targetNode, name, value) {
    if (typeof value === "undefined")
      return L.DomUtil.getStyle(targetNode, name);
  ...
  var Area = function Area(_ref) {
    var width = _ref.width,
      ...
      interpolation =
        _ref$interpolation === void 0 ? "curveLinear" : _ref$interpolation;
    return d3
      .area()
      .curve(
  ...
  var Axis = function Axis(_ref3) {
    ...
    return function (g) {
      ...
      var axisScale = d3[
        "axis" +
          position.replace(/\b\w/g, function (l) {
  ...
  var MouseFocusLabel = function MouseFocusLabel(_ref9) {
    ...
    return function (g) {
      ...
      if (xCoord)
        xAlign = xCoord + (xCoord < width / 2 ? 10 : -bbox.width - 10);
      if (yCoord)
        yAlign = Math.max(
          yCoord - bbox.height,
          L.Browser.webkit ? 0 : -Infinity,
        );
      rect
  ...
  var Chart = function Chart(_ref15) {
    ...
    focusG.append("svg:line").call(
      MouseFocusLine({
        xCoord: 0,
        height: _height,
      }),
    );
    focusG.append("g").call(
      MouseFocusLabel({
        xCoord: 0,
        yCoord: 0,
        height: _height,
  ...
  var Chart$1 = L.Class.extend({
    ...
    _initRuler: function _initRuler() {
      ...
      var label = function label(e, d) {
        ...
        if (y >= yMax || y <= 0)
          _this5._dragG.select(".horizontal-drag-label").text("");
      ...
      var position = function position(e, d) {
        ...
        _this5._container
          ...
          .text(formatNum(z) + " " + _this5._yLabel);

        _this5.fire("ruler_filter", {
          coords:
            yCoord < yMax && yCoord > 0 ? _this5._findCoordsForY(yCoord) : [],
        });
      };
    ...
    _findCoordsForY: function _findCoordsForY(y) {
      ...
      var coords = list.reduce(function (array, _, curr) {
        ...
        if (list[next] !== list[curr] + 1 || next === list.length) {
          array.push(
            list.slice(start, next).map(function (i) {
    ...
    _showDiagramIndicator: function _showDiagramIndicator(item, xCoordinate) {
      ...
      this._focuslabel.call(
        MouseFocusLabel({
          ...
          width: this._width(),
          labelX:
            d3.format("." + opts.decimalsX + "f")(item[opts.xAttr]) +
            " " +
            this._xLabel,
          labelY:
            d3.format("." + opts.decimalsY + "f")(item[opts.yAttr]) +
            " " +
            this._yLabel,
        }),
      ...
      if (this.options.marker == "elevation-line") {
        ...
        this._focuslabel.call(
          HeightFocusLabel({
            ...
            yCoord: normalizedY,
            label:
              d3.format("." + opts.decimalsY + "f")(point[opts.yAttr]) +
              " " +
              this._yLabel,
          }),
        );
  ...
  var Summary = L.Class.extend({
    initialize: function initialize(opts, control) {
      ...
      var summary = (this._container = create(
        "div",
    ...
    append: function append(className, label, value) {
      this._container.innerHTML += '<span class="'
        .concat(className, '"><span class="summarylabel">')
        .concat(label, '</span><span class="summaryvalue">')
        .concat(value, "</span></span>");
      return this;
    },
    _registerSummary: function _registerSummary(data) {
      for (var i in data) {
        this.append(
          i,
          L._(data[i].label),
          typeof data[i].value !== "function"
            ? data[i].value
            : data[i].value(this.control.track_info),
        );
      }
    },
  });
  ...
  var Elevation = (L.Control.Elevation = L.Control.extend({
    ...
    addData: function addData(d, layer) {
      ...
      Elevation._d3LazyLoader = lazyLoader(
        this.__D3,
        (typeof d3 === "undefined" ? "undefined" : _typeof(d3)) !== "object" ||
    ...
    loadXML: function loadXML(data) {
      ...
      Elevation._togeojsonLazyLoader = lazyLoader(
        this.__TOGEOJSON,
        ...
        if (!(type in toGeoJSON)) {
          type =
            xml.documentElement.tagName == "TrainingCenterDatabase"
              ? "tcx"
              : "gpx";
        }
        ...
        if (name[0]) {
          ...
          geojson.name = _this3._downloadURL
            .split("/")
            .pop()
            .split("#")[0]
            .split("?")[0];
    ...
    onAdd: function onAdd(map) {
      ...
      var container = (this._container = create(
        "div",
      ...
      Elevation._d3LazyLoader = lazyLoader(
        ...
        map
          .on("zoom viewreset zoomanim", _this5._hideMarker, _this5)
          .on("resize", _this5._resetView, _this5)
          .on("resize", _this5._resizeChart, _this5)
          .on("rotate", _this5._rotateMarker, _this5)
          .on("mousedown", _this5._resetDrag, _this5);
        ...
        _this5
          .on("eledata_added eledata_loaded", _this5._updateChart, _this5)
          .on("eledata_added eledata_loaded", _this5._updateSummary, _this5);
    ...
    _initLayer: function _initLayer() {
      ...
      this._layers.on("layeradd layerremove", function (e) {
        ...
        toggleClass(
          node,
          _this9.options.polyline.className + " " + _this9.options.theme,
          e.type == "layeradd",
        );
        ...
        toggleEvent(
          layer,
          "mousemove",
          _this9._mousemoveLayerHandler.bind(_this9),
          e.type == "layeradd",
        );
        ...
        toggleEvent(
          layer,
          "mouseout",
          _this9._mouseoutHandler.bind(_this9),
          e.type == "layeradd",
        );
      });
    },
    ...
    _resizeChart: function _resizeChart() {
      ...
      if (opts.responsive) {
        ...
        if (opts.detached) {
          ...
          newWidth =
            opts._maxWidth > clientWidth ? clientWidth - 30 : opts._maxWidth;
        }
    ...
    _toggle: function _toggle() {
      if (hasClass(this._container, "elevation-expanded")) this._collapse();
      else this._expand();
    },
    ...
    _toggleChartHandler: function _toggleChartHandler(e) {
      ...
      this._layers.eachLayer(function (layer) {
        ...
        toggleClass(
          node,
          _this16.options.polyline.className + " " + _this16.options.theme,
          _this16._chartEnabled,
        );
      }); // toggle option value (eg. altitude = { 'disabled' || 'disabled' })

      this.options[name] =
        !enabled && this.options[name] == "disabled" ? "enabled" : "disabled"; // remove marker on empty chart
  ...
  Elevation.addInitHook(function () {
    ...
    this.on("eledata_loaded", function (e) {
      ...
      map.once("layeradd", function (e) {
        return (
          _this18.options.autofitBounds && _this18.fitBounds(layer.getBounds())
        );
      });
      ...
      if (L.Browser.mobile) {
        ...
        Elevation._geomutilLazyLoader = lazyLoader(
          this.__LGEOMUTIL,
          ...
          if (_this18.options.almostOver) {
            Elevation._almostoverLazyLoader = lazyLoader(
              _this18.__LALMOSTOVER,
              typeof L.Handler.AlmostOver !== "function" ||
                !_this18.options.lazyLoadJS,
          ...
          if (_this18.options.distanceMarkers) {
            Elevation._distanceMarkersLazyLoader = lazyLoader(
              _this18.__LDISTANCEM,
  ...
  Elevation.addInitHook(function () {
    ...
    this._registerSummary({
      maxele: {
        ...
        value: function value(track) {
          return (
            (track.elevation_max || 0).toFixed(2) + "&nbsp;" + _this._yLabel
          );
        },
      },
      minele: {
        ...
        value: function value(track) {
          return (
            (track.elevation_min || 0).toFixed(2) + "&nbsp;" + _this._yLabel
          );
        },
      },
    });
    ...
    this.on("elepoint_added", function (e) {
      ...
      if (!e.point.meta || !e.point.meta.time) {
        ...
        e.point.meta.time = new Date(
          i > 0
            ? data[i - 1].time.getTime() +
              (delta / speed) * this._timeFactor * 1000
            : Date.now(),
        );
      }
  ...
  Elevation.addInitHook(function () {
    ...
    this.on("eledata_updated", function (e) {
      ...
      if (!isNaN(z)) {
        ...
        else if (deltaZ < 0) tDes -= deltaZ; // slope in % = ( height / length ) * 100
  ...
  Elevation.addInitHook(function () {
    ...
    var speed = {};
    speed.label =
      opts.speedLabel || L._(this.options.imperial ? "mph" : "km/h");
    ...
    this.on("elepoint_added", function (e) {
      ...
      if (deltaT > 0) {
        ...
        speed =
          Math.abs((delta / deltaT) * this._timeFactor) * this._speedFactor;
  ...
  Elevation.addInitHook(function () {
    ...
    var acceleration = {};
    acceleration.label =
      opts.accelerationLabel || L._(this.options.imperial ? "ft/s²" : "m/s²");
    ...
    this.on("elepoint_added", function (e) {
      ...
      if (this.options.accelerationDeltaMax) {
        ...
        if (Math.abs(deltaA) > maxDeltaS) {
          acceleration =
            data[i - 1].acceleration + maxDeltaS * Math.sign(deltaA);
        }
    ...
    this._registerSummary({
      minacceleration: {
        ...
        value: function value(track) {
          return (
            Math.round(track.acceleration_min || 0) +
            "&nbsp;" +
            acceleration.label
          );
        },
      ...
      maxacceleration: {
        ...
        value: function value(track) {
          return (
            Math.round(track.acceleration_max || 0) +
            "&nbsp;" +
            acceleration.label
          );
        },
      ...
      avgacceleration: {
        ...
        value: function value(track) {
          return (
            Math.round(track.acceleration_avg || 0) +
            "&nbsp;" +
            acceleration.label
          );
        },
