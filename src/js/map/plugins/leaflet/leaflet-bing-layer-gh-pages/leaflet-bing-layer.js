          ...
          L.TileLayer.Bing = L.TileLayer.extend({
            ...
            initialize: function (options) {
              ...
              if (
                options &&
                options.style &&
                DYNAMIC_IMAGERY_SETS.indexOf(options.imagerySet) < 0
              ) {
                console.warn(
                  "Dynamic styles will only work with these imagerySet choices: " +
                    DYNAMIC_IMAGERY_SETS.join(", "),
                );
              }
    ...
    */
            ...
            getMetaData: function (latlng, zoom) {
              ...
              var PointMetaDataUrl = L.Util.template(
                L.TileLayer.Bing.POINT_METADATA_URL,
                {
                  bingMapsKey: this.options.bingMapsKey,
                  imagerySet: this.options.imagerySet,
                  z: zoom,
            ...
            _getAttributions: function (bbox, zoom) {
              return this._imageryProviders.reduce(function (
                ...
                for (var i = 0; i < provider.coverageAreas.length; i++) {
                  if (
                    bboxIntersect(bbox, provider.coverageAreas[i].bbox) &&
