...
class Graphics {
  ...
  createModal() {
    ...
    divContainer.appendChild(
      this.createElement("div", "graphicsd3", "graphicsd3"),
    );
  ...
  render(value) {
    ...
    if (isNaN(maxim)) {
      ...
      const yScale = d3.scaleLinear().range([height, 0]).domain([0, maxdomain]);
      ...
      const barGroups = chart.selectAll().data(datad3).enter().append("g");
