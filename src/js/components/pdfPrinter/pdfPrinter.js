...
class PdfPrinter {
  ...
  createComponent() {
    ...
    try {
      ...
      if (this.elem) {
        console.warn(
          "PdfPrinter: El componente ya existe, destruyendo el anterior",
        );
        this.destroy();
      }
  ...
  async print() {
    ...
    try {
      ...
      if (
        typeof PDFClass === "undefined" ||
        typeof window.leafletImage === "undefined"
      ) {
        throw new Error(
          "jsPDF y leaflet-image son necesarios para imprimir el mapa.",
        );
      }
      ...
      overlays = Array.from(
        mapElem.querySelectorAll(
          ".leaflet-control, .leaflet-top, .leaflet-right, .leaflet-left, .leaflet-bottom",
        ),
      );
      overlays.forEach((el) => {
      ...
      if (scaleElem) {
        ...
        scaleClone.style.cssText =
          "position:absolute;left:20px;bottom:20px;z-index:10000;";
        mapElem.appendChild(scaleClone);
      }

      // Capturar mapa con timeout configurable
      const canvas = await this.#captureMapWithTimeout(
        mapObj,
        this.config.timeout,
      );
  ...
  #getActiveLayersWithLegends() {
    try {
      if (
        ...
        gestorMenu.getActiveLayersWithoutBasemap
      ) {
  ...
  #captureMapWithTimeout(mapObj, timeout) {
    return new Promise((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        reject(
          new Error(
            `Tiempo de espera agotado (${timeout}ms) al generar la imagen del mapa. Puede que alguna capa no esté respondiendo.`,
          ),
        );
      }, timeout);
          ...
          reject(
            new Error(
              "No se pudo capturar el mapa: " +
                (err?.message || "Canvas no generado"),
            ),
          );
        } else {
          resolve(canvas);
  ...
  #addAttribution(pdf, mapObj, offsetX, offsetY, imgW, imgH) {
    try {
      ...
      if (
        mapObj &&
        mapObj.attributionControl &&
        mapObj.attributionControl._container
      ) {
        const tempDiv = document.createElement("div");
        tempDiv.innerHTML = mapObj.attributionControl._container.innerHTML;
        const texts = Array.from(tempDiv.querySelectorAll("a")).map((a) =>
          a.textContent.trim(),
        );
      ...
      if (attributionText) {
        ...
        pdf.text(attributionText, offsetX + imgW - 2, offsetY + imgH + 7, {
          align: "right",
        });
      }
    } catch (e) {
  ...
  #generateFilename() {
    ...
    const appName =
      typeof app !== "undefined" && app.title
        ? app.title.toLowerCase().replace(/[^a-z0-9]/g, "_")
        : "mapa";
  ...
  destroy() {
    try {
      ...
      if (this.state === "loading" || this.state === "processing") {
        console.warn(
          "PdfPrinter: Cancelando operación en curso durante destroy()",
        );
        this.state = "idle";
      }
  ...
  #addScaleTextToPDF(pdf, mapObj, x, y) {
    try {
      ...
      if (
        mapObj &&
        typeof mapObj.getZoom === "function" &&
        typeof mapObj.getCenter === "function"
      ) {
  ...
  #drawGraphicScale(pdf, x, y, scaleInfo) {
    try {
      ...
      pdf.text(
        scaleInfo.text,
        x + scaleInfo.barWidthMm + 2,
        y + barHeight + textOffset + 2,
      );
