class ServiceLayers {
  ...
  async handleRequest(url) {
    ...
    await fetch(this.url, options)
      .then((res) => {
        if (!res.ok) {
          throw new Error(
            "Network response was not OK. Status code: " + res.status,
          );
        }
        return res.text();
      })
  ...
  async loadWMS(url) {
    try {
      ...
      return (this.layers = this.rawData.Capability.Layer.Layer.map((layer) => {
        ...
        let style = "";
        layer.Style ? (style = layer.Style[0].LegendURL[0]) : style;
        return {
          name: layer.Name,
          title: layer.Title,
  ...
  validateUrl(url) {
    ...
    return {
      ...
      capability:
        a.origin + a.pathname + "?service=WMS&request=GetCapabilities",
    };
  }
}
